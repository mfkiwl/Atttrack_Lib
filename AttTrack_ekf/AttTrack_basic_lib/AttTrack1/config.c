...
int config_set_ekf_angle_err_init(double num)
{
	json_object_dotset_number(s_cfg_obj, "imu.angle_err_init", num);
	save_config();
	return 1;
}
double config_get_ekf_angle_err_init()
{
	return json_object_dotget_number(s_cfg_obj, "imu.angle_err_init");
}

int config_set_ekf_bias_err_init(double num)
{
	json_object_dotset_number(s_cfg_obj, "imu.bias_err_init", num);
	save_config();
	return 1;
}
double config_get_ekf_bias_err_init()
{
	return json_object_dotget_number(s_cfg_obj, "imu.bias_err_init");
}

int config_set_ekf_gnoisestd(double num)
{
	json_object_dotset_number(s_cfg_obj, "imu.gnoisestd", num);
	save_config();
	return 1;
}
double config_get_ekf_gnoisestd()
{
	return json_object_dotget_number(s_cfg_obj, "imu.gnoisestd");
}
...
int config_set_ekf_angle_flag(int num)
{
	json_object_dotset_number(s_cfg_obj, "imu.angle_flag", num);
	save_config();
	return 1;
}
int config_get_ekf_angle_flag()
{
	return json_object_dotget_number(s_cfg_obj, "imu.angle_flag");
}
...
void generate_default_config_Angle()
{
	...
	config_set_gyo_noisex(0.0619);     //deg
	config_set_gyo_noisey(0.0619);     //deg
	config_set_gyo_noisez(0.0619);     //deg


	config_set_gyo_stabilityx(2.0);   //dph
	config_set_gyo_stabilityy(2.0);   //dph
	config_set_gyo_stabilityz(2.0);   //dph

	config_set_ekf_angle_err_init(0.01);
	config_set_ekf_bias_err_init(0.1);
	config_set_ekf_gnoisestd(0.5);

	config_set_ekf_strategy(1);
	config_set_ekf_angle_flag(1);
...
void get_conf(Config_t* cfg)
{
	...
	cfg->angle_err_init = config_get_ekf_angle_err_init();
	cfg->bias_err_init = config_get_ekf_bias_err_init();
	cfg->gnoisestd = config_get_ekf_gnoisestd();

	cfg->ekf_strategy = config_get_ekf_strategy();
	cfg->angle_flag = config_get_ekf_angle_flag();
